...
Ext.Ajax.request({scope:this,method:'POST',url:'api/session',params:form.getValues(),callback:function(options,success,response){Ext.get('spinner').setVisible(!1);success?(Traccar.app.setUser(Ext.decode(response.responseText)),this.fireViewEvent('login')):(this.getView().setVisible(!0),401===response.status?Traccar.app.showError(Strings.loginFailed):Traccar.app.showError(response.responseText))}}))},logout:function(){Ext.util.Cookies.clear('user');Ext.util.Cookies.clear('password');Ext.Ajax.request({scope:this,
method:'DELETE',url:'api/session',callback:function(){window.location.reload()}})},onSelectLanguage:function(selected){selected=selected.getValue();var url=window.location.href;if(0<=url.indexOf('locale\x3d')){var prefix=url.substring(0,url.indexOf('locale'));url=url.substring(url.indexOf('locale'));url=url.substring(url.indexOf('\x3d')+1);url=0<=url.indexOf('\x26')?url.substring(url.indexOf('\x26')):'';url=prefix+'locale\x3d'+selected+url}else url=0>url.indexOf('?')?url+('?locale\x3d'+selected):
...
params:{email:text},callback:function(options,success,response){success?Traccar.app.showToast(Strings.loginResetSuccess):Traccar.app.showError(response.responseText)}})})}});
...
buttons:[{text:Strings.loginReset,handler:'onResetClick',reference:'resetButton'},{text:Strings.loginRegister,handler:'onRegisterClick',reference:'registerButton'},{text:Strings.loginLogin,handler:'onLoginClick'}]});Ext.define('Traccar.view.MainController',{extend:Ext.app.ViewController,alias:'controller.mainController',init:function(){this.lookupReference('reportView').setHidden(Traccar.app.getPreference('disableReports',!1));this.lookupReference('eventsView').setHidden(Traccar.app.getBooleanAttributePreference('ui.disableEvents'))}});
...
{text:Strings.deviceContact,dataIndex:'contact',hidden:!0},{text:Strings.groupDialog,dataIndex:'groupId',hidden:!0,filter:{type:'list',labelField:'name',store:'Groups'},renderer:Traccar.AttributeFormatter.getFormatter('groupId')},{text:Strings.sharedDisabled,dataIndex:'disabled',renderer:Traccar.AttributeFormatter.getFormatter('disabled'),hidden:!0,filter:'boolean'},{text:Strings.deviceStatus,dataIndex:'status',filter:{type:'list',labelField:'name',store:'DeviceStatuses'},renderer:function(value){return value&&
(value=Ext.getStore('DeviceStatuses').getById(value))?value.get('name'):null}},{text:Strings.deviceLastUpdate,dataIndex:'lastUpdate',renderer:Traccar.AttributeFormatter.getFormatter('lastUpdate')}]}});
...
Traccar.Style.reconnectTimeout)};protocol.onmessage=function(event){event=Ext.decode(event.data);event.devices&&self.updateDevices(event.devices);event.positions&&(self.updatePositions(event.positions,first),first=!1);event.events&&self.updateEvents(event.events)}},updateDevices:function(array){var i,entity;var store=Ext.getStore('Devices');for(i=0;i<array.length;i++)(entity=store.getById(array[i].id))&&entity.set({status:array[i].status,lastUpdate:array[i].lastUpdate},{dirty:!1})},updatePositions:function(array,
first){var i,entity,device;var store=Ext.getStore('LatestPositions');for(i=0;i<array.length;i++)(entity=store.findRecord('deviceId',array[i].deviceId,0,!1,!1,!0))?entity.set(array[i]):store.add(Ext.create('Traccar.model.Position',array[i])),Ext.getStore('Events').findRecord('positionId',array[i].id,0,!1,!1,!0)&&Ext.getStore('EventPositions').add(Ext.create('Traccar.model.Position',array[i]));first&&((array=Ext.Object.fromQueryString(window.location.search).deviceId)&&(device=Ext.getStore('VisibleDevices').findRecord('uniqueId',
array,0,!1,!0,!0))&&this.fireEvent('selectdevice',device,!0),device||this.zoomToAllDevices())},updateEvents:function(array){var i,device;var store=Ext.getStore('Events');for(i=0;i<array.length;i++)store.insert(0,array[i]),(device=Ext.getStore('Devices').getById(array[i].deviceId))?(this.soundPressed()&&this.beep(),Traccar.app.showToast(array[i].text,device.get('name'))):Traccar.app.showToast(array[i].text)},zoomToAllDevices:function(){var lat=Traccar.app.getPreference('latitude',0);var lon=Traccar.app.getPreference('longitude',
0);var zoom=Traccar.app.getPreference('zoom',0);0===lat&&0===lon&&0===zoom&&this.fireEvent('zoomtoalldevices')}});
